...
export const getByDate = async (date) => {
  const { data, error } = await supabase.from('daily_expenses').select('*').eq('date', date)
...
export const update = async (id, payload) => {
  const { data, error } = await supabase.from('daily_expenses').update(payload).eq('id', id).select().single()
  if (error) throw error
  return data
}
